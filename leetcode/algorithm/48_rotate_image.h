#pragma once

//  https://leetcode.com/problems/rotate-image/
/*
* 48. Rotate Image
* 
* You are given an n x n 2D matrix representing an image, rotate the image by 90
* degrees (clockwise).
* 
* You have to rotate the image in-place, which means you have to modify the input 2D
* matrix directly. DO NOT allocate another 2D matrix and do the rotation.
*
* Example 1:
* Input: matrix = [
*   [1,2,3],
*   [4,5,6],
*   [7,8,9]]
* Output: [
*   [7,4,1],
*   [8,5,2],
*   [9,6,3]]
*
* Example 2:
* Input: matrix = [
*   [5,1,9,11],
*   [2,4,8,10],
*   [13,3,6,7],
*   [15,14,12,16]]
* Output: [
*   [15,13,2,5],
*   [14,3,4,1],
*   [12,6,8,9],
*   [16,7,10,11]]
*
* Constraints:
*    n == matrix.length == matrix[i].length
*    1 <= n <= 20
*    -1000 <= matrix[i][j] <= 1000
*/

#include <vector>
#include <gtest/gtest.h>
#include "../util/std_util.h"

namespace RotateImage {
    class Solution {
    public:
        void rotate(std::vector<std::vector<int>>& matrix) {
            int sz = matrix.size();
            for (int i = 0; i < (sz >> 1); i++) {
                for (int j = i; j < sz - i - 1; j++) {
                    int v = matrix[i][j];
                    matrix[i][j] = matrix[sz - j - 1][i];
                    matrix[sz - j - 1][i] = matrix[sz - i - 1][sz - j - 1];
                    matrix[sz - i - 1][sz - j - 1] = matrix[j][sz - i - 1];
                    matrix[j][sz - i - 1] = v;
                }
            }
        }
    };

    bool testRotate(std::initializer_list<const char*> input, std::initializer_list<const char*> expected) {
        std::vector<std::vector<int>> inputMatrix = StdUtil::generateIntVectorMatrix(input);
        std::vector<std::vector<int>> expectedMatrix = StdUtil::generateIntVectorMatrix(expected);
        Solution().rotate(inputMatrix);
        return StdUtil::compareEqualVectorMatrix(inputMatrix, expectedMatrix);
    }
}


TEST(_48_RotateImage, SimpleCases) {
    /*
    * Example 1:
    * Input: matrix = [
    *   [1,2,3],
    *   [4,5,6],
    *   [7,8,9]]
    * Output: [
    *   [7,4,1],
    *   [8,5,2],
    *   [9,6,3]]
    */
    EXPECT_TRUE(RotateImage::testRotate(
        {
            "[1,2,3]",
            "[4,5,6]",
            "[7,8,9]"
        }, 
        {
            "[7,4,1]",
            "[8,5,2]",
            "[9,6,3]"
        }));

    /*
    * Example 2:
    * Input: matrix = [
    *   [5,1,9,11],
    *   [2,4,8,10],
    *   [13,3,6,7],
    *   [15,14,12,16]]
    * Output: [
    *   [15,13,2,5],
    *   [14,3,4,1],
    *   [12,6,8,9],
    *   [16,7,10,11]]
    */
    EXPECT_TRUE(RotateImage::testRotate(
        {
            "[5,1,9,11]",
            "[2,4,8,10]",
            "[13,3,6,7]",
            "[15,14,12,16]"
        },
        {
            "[15,13,2,5]",
            "[14,3,4,1]",
            "[12,6,8,9]",
            "[16,7,10,11]"
        }));
}